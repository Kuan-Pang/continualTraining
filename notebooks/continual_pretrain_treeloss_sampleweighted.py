...
class TreeLoss(torch.nn.Module):
    ...
    def forward(self, pred, target):
        ...
        target_node_id = self.nodes[target_cpu]
        pred_node_id = self.nodes[np.argmax(pred_cpu, axis=1)]
        
        # dist for each prediction
        dist_array = np.zeros(len(target_node_id))
        for i in range(len(target_node_id)):
            dist_array[i] = self.lca_dist(target_node_id[i], pred_node_id[i])
            
        # normalize the distance
        total_dist = np.sum(dist_array)
        dist_array /= total_dist
        
        # weight each sample by the distance in loss calculation
        weighted_ce = torch.zeros(pred.shape[0])
        ...
        return ce_loss + self.lamda * weighted_ce
